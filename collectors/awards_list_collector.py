# 17. 수상작 목록 조회 (prfawadService) - 페이지 반복 수집 예시
import requests
import pandas as pd
import xmltodict
import os
...
def collect_prfawad_list(
    ...
    """
    17. 수상작 목록 조회 (prfawadService) - 모든 페이지(cpage)를 반복 수집하여 DataFrame 병합
    ...
    for page in range(1, max_pages + 1):
        ...
        if response.status_code != 200:
            print(f"[collect_prfawad_list_all] HTTP Error {response.status_code}, page={page}")
        ...
        try:
            ...
            print(f"[collect_prfawad_list_all] XML 파싱 오류: {e}")
            break

        # OpenAPI 오류 태그 검사 (예: <OpenAPI_ServiceResponse>..)
        if "OpenAPI_ServiceResponse" in data_dict:
            print("[collect_prfawad_list_all] OpenAPI Error 응답:", data_dict)
            break

        # 4) 실제 목록 추출
        dbs = data_dict.get("dbs")
        if not dbs:
            print(f"[collect_prfawad_list_all] 'dbs' 태그 없음 -> 종료(page={page})")
            break

        items = dbs.get("db")
        if not items:
            print(f"[collect_prfawad_list_all] 'db' 태그 비었음 -> 종료(page={page})")
            break
        ...
        for item in items:
            page_records.append({
                ...
                "awards":    item.get("awards")     # 수상실적
            })

        df_page = pd.DataFrame(page_records)
        if df_page.empty:
            print(f"[collect_prfawad_list_all] 페이지 {page} 결과 0건 -> 종료")
            break
        ...
        if len(df_page) < rows:
            print(f"[collect_prfawad_list_all] (page={page}) 마지막 페이지로 판단 -> 종료")
            break
